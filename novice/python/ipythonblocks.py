...
"""
	if not vars().has_key('plt'):
		from matplotlib import pyplot as plt
		from matplotlib import collections as mcoll
		from matplotlib import patches as mpatches
	if not vars().has_key('numpy'):
		import numpy
	...
	#each item is a list corresponding to the row
	for i,row in enumerate(self._to_simple_grid()):
		for j,(r,g,b,s) in enumerate(row):
			col = (r/255.,g/255.,b/255.,1.)
			_patches.append(mpatches.Rectangle((j*s,-i*s), s,s, facecolor=col, edgecolor='white'))
	
	_coll = mcoll.PatchCollection(_patches, match_original=True)
	_ax.add_collection(_coll)
